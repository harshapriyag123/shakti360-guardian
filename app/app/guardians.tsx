...
export default function Guardians() {
  ...
  async function addAndSend() {
    try {
      ...
      const created = await authPost<Guardian>("/guardians", { name: name.trim(), relationship: relationship.trim(), phone: phone.trim() || null, email: email.trim() || null, priority: Math.min(items.length + 1, 5), journey_started: true, missed_checkin: true, sos: true, live_location: true });
  ...
  return <Screen><Eyebrow>TRUSTED GUARDIAN CIRCLE</Eyebrow><Title subtitle="You decide who receives journey, missed check-in, SOS, and temporary location updates.">People you trust</Title><Card tone="mint"><Text style={{ color: colors.primaryDark, fontWeight: "900" }}>Session-based, never permanent</Text><Text style={{ color: colors.muted, lineHeight: 20 }}>Invitation links expire after 24 hours. Location access expires after each safety session.</Text></Card>
    ...
    {busy ? <Loading label="Sending through secure delivery providers…" /> : null}{notice ? <Card tone="mint"><Text accessibilityLiveRegion="polite" style={{ color: colors.primaryDark, lineHeight: 20, fontWeight: "700" }}>{notice}</Text></Card> : null}{error ? <ErrorBanner message={error} retry={load} /> : null}<Text style={{ color: colors.muted, fontSize: 12, lineHeight: 18 }}>“Queued” means the provider accepted the message, not that the recipient opened it. Carrier and mailbox delivery can still fail.</Text></Screen>;
}
